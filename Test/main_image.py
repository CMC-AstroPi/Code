...
file_info_time.info(',ID_Photo,IsDayRunTime,CalculateAreascalingRunTime,MachineLearningRuntime')
# -------------------------------------------------------
...
def createNamePhoto():
    return('-img_'+str(getNumerPhoto())+'.jpg')
# -------------------------------------------------------
...
def makePhoto(nameImage):
    ...
    if(v_day):
        ...
        try:
            initial_time = getTimer()
            data_photo = calculate_areascaling(nameImage)
            times.append(str(getTimer() - initial_time))
            ...
            if(value == 1.):
                ...
                new_name = 'other'
            os.rename(nameImage, new_name+nameUltimatePhoto)
            saveData(new_name)
        except Exception as e:
...
# main function
def run():
    nameImages = extractImages("./image_folder")
    for image in nameImages:
        makePhoto('./image_folder/'+image)
